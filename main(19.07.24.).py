...
def data_frame(
    ...
    for index in range(1, n):
        ...
        # v = [ dx, dy, A2/A[%] ]
        """ We find the object closest to our objective's  location, as in:
            https://stackoverflow.com/questions/39107896/
            efficiently-finding-the-closest-coordinate-pair-from-a-set-in-python """
        deltas = stats2[:, :2] - np.asarray([x, y])
        deltas = np.einsum('ij, ij -> ij', deltas, deltas)
        deltas = np.sum(deltas, axis=1)
        min_dist_index = np.argmin(deltas)

        dA = round(stats2[min_dist_index][4] * 100 / A)  # area change
        # print(dA, stats2[min_dist_index][4], A)
        flow_vectors[old_center] = (32767 + sx - x + fx, 32767 + sy - y + fy, dA)  # we add (2^16-1)//2 to avoid
        # "negative overflow"
...
def main():
    ...
    flow_vector_field = np.zeros(handler.image.shape + (3,), np.uint16)
    ...
    for frame_n in range(handler.n_frames - 1):  # handler.n_frames-1
        local_cache = data_frame(
            ...
            flow_vectors=flow_vector_field,
            flow_lines_visual=flow_lines_visual
        )

    """ Saving the flow_lines image for debugging's sake: """
    ...
    with open(f'{handler.directory}{handler.filename}[flow_field].npy', 'wb') as f:
        np.save(f, flow_vector_field)

    """ Loading the data back into the flow_lines image and saving, also for debugging's sake: """
    temp = np.load(f'{handler.directory}{handler.filename}[flow_field].npy')
    temp2 = np.zeros((640, 640, 3), np.uint8)
    for y in range(640):
        for x in range(640):
            if tuple(temp[y, x]) != (0, 0, 0):
                p2 = tuple(temp[y, x][:2] - np.asarray([32767, 32767]) + np.asarray([y, x]))
                print(p2 + (temp[y, x][-1],))
